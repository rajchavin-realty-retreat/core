...
export default function SchemaBuilder({ activeWorkspace, entities, fetchEntities, setIsCreatingEntity, setActiveTab }) {
  const [entityName, setEntityName] = useState('');
  
  // Added dependentField and conditionMap to the default state
    ...
    try {
      ...
      const formattedFields = entityFields.map(field => {
        ...
        if (field.type === 'formula') {
          if (!field.formula) throw new Error(`You must define an equation for the "${field.name}" computed column.`);
          return { name: field.name, type: field.type, formula: field.formula };
        }
        
        // --- COMPILE CONDITIONAL FORMULAS FOR BACKEND ---
        if (field.type === 'conditional-formula') {
          if (!field.dependentField) throw new Error(`Select a dependent dropdown for "${field.name}".`);
      ...
      setEntityName('');
      setEntityFields([{ name: '', type: 'text', optionsArray: [], tempOption: '', targetEntity: '', formula: '', dependentField: '', conditionMap: {} }]);
  ...
  // Specific updater for the conditional map
        ...
        <div className="space-y-3">
          ...
          {entityFields.map((field, index) => (
            <div key={index} className="flex flex-col gap-2 p-3 bg-zinc-50 border border-zinc-100 rounded-lg">
              <div className="flex flex-col sm:flex-row gap-2 items-start sm:items-center">
                ...
                <div className="flex gap-2 w-full sm:w-auto">
                  <select value={field.type} onChange={(e) => updateField(index, 'type', e.target.value)} className="flex-1 sm:w-48 px-3 py-1.5 text-sm border border-zinc-200 rounded-md bg-white outline-none focus:border-indigo-500">
                    ...
                    <option value="conditional-formula">Conditional Logic (Switch)</option> 
                  </select>
                  {entityFields.length > 1 && (
                    <button type="button" onClick={() => removeFieldFromSchema(index)} className="p-1.5 text-zinc-400 hover:text-red-500 bg-white border border-zinc-200 rounded-md">
                      ✕
                    </button>
                  )}
                </div>
              ...
              {/* DYNAMIC UI FOR CONDITIONAL FORMULA */}
                ...
                <div className="pt-3 border-t border-zinc-200 mt-2 space-y-3">
                  <div>
                    ...
                    <select required value={field.dependentField || ''} onChange={(e) => updateField(index, 'dependentField', e.target.value)} className="w-full sm:w-1/2 px-3 py-1.5 text-sm border border-indigo-200 rounded-md bg-indigo-50/30 text-indigo-900 outline-none focus:border-indigo-500">
                      ...
                      {entityFields.filter(f => f.type === 'dropdown' && f.name !== field.name).map((f, i) => (
                    ...
                    <div className="space-y-2 bg-white p-3 border border-indigo-100 rounded-md">
                      ...
                      {entityFields.find(f => f.name === field.dependentField)?.optionsArray.map((opt, optIdx) => (
                        <div key={optIdx} className="flex flex-col sm:flex-row sm:items-center gap-2">
                          ...
                          <input 
                            type="text" 
                            placeholder="e.g., {Salary} * 0.10"
                            value={field.conditionMap?.[opt] || ''}
                            onChange={(e) => updateConditionMap(index, opt, e.target.value)}
                            className="flex-1 font-mono text-xs px-2 py-1.5 border border-emerald-200 rounded bg-emerald-50/30 outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500"
                          />
                        </div>
                      ))}
                      {entityFields.find(f => f.name === field.dependentField)?.optionsArray.length === 0 && (
                        <p className="text-xs text-red-500">The selected dropdown has no options defined yet!</p>
                      )}
                    </div>
                  )}
                </div>
              ...
              {field.type === 'formula' && (
                <div className="pt-2 border-t border-zinc-200 mt-1">
                  ...
                  <input type="text" required placeholder="e.g., {Price} * {Quantity}" value={field.formula || ''} onChange={(e) => updateField(index, 'formula', e.target.value)} className="w-full font-mono text-xs px-3 py-2 border border-emerald-200 rounded-md bg-emerald-50/30 text-emerald-900 outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500" />
                </div>
              )}
