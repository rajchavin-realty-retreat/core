...
export default function Dashboard() {
  ...
  // --- UI STATE ---
  const [activeTab, setActiveTab] = useState('overview'); // 'overview' or 'databases'
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false); // NEW: Mobile Sidebar Toggle
  ...
  const handleCreateEntity = async (e) => {
    ...
    try {
      ...
      setActiveTab('databases'); // Switch back to databases view after creation
  ...
  // Helper to change workspace and close mobile menu
  ...
  return (
    <div className="flex h-screen w-full bg-[#FBFBFC] text-zinc-900 font-sans overflow-hidden">
      ...
      {/* ========================================== */}
      {/* LEFT SIDEBAR (Responsive Mobile Drawer)    */}
      {/* ========================================== */}
            ...
            <ul className="space-y-0.5">
              {activeOrAcceptedWorkspaces.map(ws => (
                <li key={ws._id} onClick={() => selectWorkspace(ws)}
                  ...
                  {activeWorkspace?._id === ws._id && currentUserRole === 'owner' && (
            ...
            <div>
              ...
              <form onSubmit={handleInvite} className="px-2 space-y-1 mb-3">
                ...
                <div className="flex gap-1">
                  <select value={inviteRole} onChange={(e) => setInviteRole(e.target.value)} className="flex-1 px-1 py-1 text-xs border border-zinc-200 rounded bg-white outline-none">
                    <option value="editor">Editor</option>
                    <option value="viewer">Viewer</option>
                    <option value="admin">Admin</option>
                  </select>
                  <button type="submit" disabled={isInviting} className="bg-zinc-100 text-zinc-700 border border-zinc-200 px-3 py-1 rounded text-xs font-medium hover:bg-zinc-200 transition-colors disabled:opacity-50">
                    Invite
                  </button>
                </div>
              </form>
                ...
                {activeWorkspace.members.map((m, i) => (
                  <li key={i} className="flex justify-between items-center py-1">
                    <div className="truncate flex-1">
                      <span className="text-zinc-700 font-medium">{m.user?.name || 'Pending'}</span>
                    ...
                    <span className="text-[9px] text-zinc-500 uppercase font-semibold">{m.role}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <header className="border-b border-zinc-200 bg-white flex flex-col flex-shrink-0">
          <div className="h-14 flex items-center justify-between px-4 sm:px-6">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                {activeWorkspace && (
                  <span className="hidden sm:inline-block px-2 py-0.5 rounded-full bg-zinc-100 border border-zinc-200 text-[10px] uppercase font-bold text-zinc-500 tracking-wider">
                    {currentUserRole}
                  </span>
                )}
              </div>
            ...
            {(currentUserRole === 'owner' || currentUserRole === 'admin') && activeWorkspace && (
          ...
          {activeWorkspace && (
            <div className="flex gap-6 px-4 sm:px-6 mt-auto overflow-x-auto hide-scrollbar">
              <button 
                onClick={() => { setActiveTab('overview'); setIsCreatingEntity(false); }}
                className={`pb-3 text-sm font-semibold transition-colors border-b-2 whitespace-nowrap ${activeTab === 'overview' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-zinc-500 hover:text-zinc-800'}`}
              >
                Overview
              </button>
              <button 
                onClick={() => setActiveTab('databases')}
                className={`pb-3 text-sm font-semibold transition-colors border-b-2 whitespace-nowrap ${activeTab === 'databases' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-zinc-500 hover:text-zinc-800'}`}
              >
                Databases
              </button>
            </div>
        ...
        <div className="flex-1 overflow-auto p-4 sm:p-6">
          {activeWorkspace ? (
            <div className="max-w-6xl mx-auto space-y-6">
              ...
              {activeTab === 'overview' && !isCreatingEntity && (
  <AnalyticsView entities={entities} activeWorkspace={activeWorkspace} />
)}
              ...
              {/* SCHEMA BUILDER UI (Responsive Adjustments) */}
                    ...
                    <div className="space-y-3">
                      ...
                      {entityFields.map((field, index) => (
                        <div key={index} className="flex flex-col gap-2 p-3 bg-zinc-50 border border-zinc-100 rounded-lg">
                          {/* Flex goes to column on mobile, row on larger screens */}
                          <div className="flex flex-col sm:flex-row gap-2 items-start sm:items-center">
                            ...
                            <div className="flex gap-2 w-full sm:w-auto">
                              <select value={field.type} onChange={(e) => updateField(index, 'type', e.target.value)} className="flex-1 sm:w-40 px-3 py-1.5 text-sm border border-zinc-200 rounded-md bg-white outline-none focus:border-indigo-500">
                                <option value="text">Short Text</option>
                                <option value="textarea">Long Text</option>
                                <option value="number">Number</option>
                                <option value="date">Date</option>
                                <option value="datetime">Date & Time</option> {/* <-- ADD THIS LINE */}
                                <option value="checkbox">Checkbox</option>
                                <option value="dropdown">Dropdown</option>
                                <option value="media">File Upload</option>
                                <option value="media-multiple">Image Gallery (Multiple)</option>
                              </select>
            ...
            <div className="flex flex-col items-center justify-center h-full text-center max-w-sm mx-auto px-4">
              ...
              <button onClick={() => setIsMobileMenuOpen(true)} className="mt-6 lg:hidden bg-white border border-zinc-200 text-zinc-700 px-4 py-2 rounded-md text-sm font-medium shadow-sm">
                Open Menu
              </button>
            </div>
          )}
