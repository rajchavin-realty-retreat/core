...
export default function RecordView() {
  ...
  // --- THE FORMULA ENGINE ---
  // Brought in so formulas calculate LIVE even if not saved in the DB yet
  ...
  // Notice we pass the full 'recordData' object so formulas can read sibling fields!
  const renderFieldValue = (field, recordData) => {
    
    // Auto-compute formulas LIVE to prevent the "Empty" bug
    if (field.type === 'formula' || field.type === 'conditional-formula') {
      ...
      return (
        <span className="font-mono font-medium text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100 whitespace-nowrap shadow-sm">
          {computed}
        </span>
      );
    }
    ...
    switch (field.type) {
      ...
      case 'media-multiple':
        ...
        return <div className="flex gap-2 flex-wrap mt-1">{value.map((url, i) => <a key={i} href={url} target="_blank" rel="noreferrer"><img src={url} alt={`Gallery-${i}`} className="h-10 w-10 object-cover rounded-md border border-zinc-200 shadow-sm hover:opacity-80 transition-opacity" /></a>)}</div>;
  ...
  // --- FILTER ENGINE ---
  const filteredActivity = relatedActivity
    .filter(group => selectedEntityFilter === 'All' || group.entitySchema._id === selectedEntityFilter)
    .map(group => {
      const lowerSearch = searchQuery.toLowerCase();
      const filteredRecords = group.records.filter(rec => {
        if (!searchQuery) return true;
        // Search across all standard fields AND computed formula fields!
        return group.entitySchema.fields.some(field => {
          let val = (field.type === 'formula' || field.type === 'conditional-formula') 
            ? computeFormula(rec.data, getActiveFormula(field, rec.data)) 
            : rec.data[field.name];
    ...
    .filter(group => group.records.length > 0); // Hide groups that don't match the search
  ...
  return (
    <div className="min-h-screen bg-[#FBFBFC] font-sans flex flex-col">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
              ...
              {mainEntity.fields.map((field, idx) => (
                <div key={idx} className="flex flex-col gap-1">
                  ...
                  <div className="text-sm">
                    {renderFieldValue(field, mainRecord.data)}
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* RIGHT COLUMN: ENTERPRISE ANALYTICS & FILTERING */}
        <div className="lg:col-span-2 space-y-6">
          
          {/* STATS ROW */}
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-white p-4 rounded-xl border border-zinc-200 shadow-sm flex flex-col justify-center">
               <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider">Total Linked</span>
               <span className="text-2xl font-bold text-zinc-900 mt-1">{totalLinkedRecords}</span>
            </div>
            <div className="bg-white p-4 rounded-xl border border-zinc-200 shadow-sm flex flex-col justify-center">
               <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider">Databases</span>
               <span className="text-2xl font-bold text-zinc-900 mt-1">{relatedActivity.length}</span>
            </div>
            <div className="bg-white p-4 rounded-xl border border-zinc-200 shadow-sm flex flex-col justify-center">
               <span className="text-[11px] font-bold text-emerald-500 uppercase tracking-wider">Last 7 Days</span>
               <span className="text-2xl font-bold text-emerald-600 mt-1">+{recentRecords}</span>
            </div>
          </div>
          ...
          {relatedActivity.length > 0 && (
            <div className="bg-white p-4 rounded-xl border border-zinc-200 shadow-sm flex flex-col gap-4">
              <div className="flex flex-col sm:flex-row gap-3 justify-between">
                ...
                <select value={sortOrder} onChange={(e) => setSortOrder(e.target.value)} className="w-full sm:w-auto bg-white border border-zinc-200 text-zinc-700 text-sm rounded-md px-3 py-2 outline-none focus:border-indigo-500 shadow-sm cursor-pointer">
              ...
              <div className="flex gap-2 overflow-x-auto hide-scrollbar pb-1">
                <button onClick={() => setSelectedEntityFilter('All')} className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-colors ${selectedEntityFilter === 'All' ? 'bg-indigo-600 text-white shadow-sm' : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200'}`}>
                  All Records ({totalLinkedRecords})
                </button>
                {relatedActivity.map((group, idx) => (
                  <button key={idx} onClick={() => setSelectedEntityFilter(group.entitySchema._id)} className={`px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-colors ${selectedEntityFilter === group.entitySchema._id ? 'bg-indigo-600 text-white shadow-sm' : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200'}`}>
                    {group.entitySchema.name} ({group.records.length})
                  </button>
                ))}
              </div>
            </div>
          ...
          {relatedActivity.length === 0 ? (
            ...
            <div className="bg-white rounded-xl border border-zinc-200 p-12 text-center">
              <p className="text-sm text-zinc-500">No records found matching your search.</p>
            </div>
          ) : (
                        ...
                        <div key={rec._id} className="bg-white p-5 rounded-xl border border-zinc-200 shadow-sm transition-all hover:border-indigo-300">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                            {activityGroup.entitySchema.fields.map((field, i) => {
                              // Don't render the field that points back to the main record (it's redundant)
                              ...
                              return (
                                <div key={i} className="flex flex-col gap-1">
                                  ...
                                  <div className="text-sm">
                                    {renderFieldValue(field, rec.data)}
                                  </div>
                                </div>
                              );
                            })}
